...
public class RoleMenu extends Menu {
    ...
    RoleMenu(Plugin plugin, Village village, VillageManager villageManager) {
        ...
        for (String path : rolesFile.getConfigurationSection("VillageRoles").getKeys(false)) {
            ...
            addItems(new MenuItem(atomicInteger.addAndGet(1),
                    ...
                    event -> {
                        ...
                        new PermissionsMenu(getPlugin(), village, villageManager, villageRole).build().open((Player) event.getWhoClicked());
                    }));
        }
    ...
    private ItemStack role(VillageRole villageRole) {
        Item item = new Item();
        List<String> lore = Message.MENU_ROLES_LORE.toList();
        ...
        item.name(Message.MENU_ROLES_TITLE.toString().replace("{role}", villageRole.getName()));
        item.lore(updated);
